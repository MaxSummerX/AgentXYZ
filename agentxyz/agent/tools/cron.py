...
from agentxyz.cron.types import CronJobState, CronSchedule


class CronTool(Tool):
    """Инструмент для планирования напоминаний и повторяющихся задач."""
    ...
    def _format_timing(schedule: CronSchedule) -> str:
        """Отформатировать расписание как понятную строку времени."""
        ...
        if schedule.kind == "at" and schedule.at_ms:
            dt = datetime.fromtimestamp(schedule.at_ms / 1000, tz=UTC)
            return f"at {dt.isoformat()}"
        return schedule.kind

    @staticmethod
    def _format_state(state: CronJobState) -> list[str]:
        """Отформатировать состояние выполнения задания как строки для отображения."""
        lines: list[str] = []
        if state.last_run_at_ms:
            last_dt = datetime.fromtimestamp(state.last_run_at_ms / 1000, tz=UTC)
            info = (
                f"  Last run: {last_dt.isoformat()} — {state.last_status or 'unknown'}"
            )
            if state.last_error:
                info += f" ({state.last_error})"
            lines.append(info)
        if state.next_run_at_ms:
            next_dt = datetime.fromtimestamp(state.next_run_at_ms / 1000, tz=UTC)
