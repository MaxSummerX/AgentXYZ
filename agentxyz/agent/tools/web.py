"""Веб-инструменты: web_search и web_fetch."""
...
if TYPE_CHECKING:
    from agentxyz.config.schema import WebSearchConfig
...
def _normalize(text: str) -> str:
    ...
    Returns:
        Текст с нормализованными пробелами и переносами строк.
    """
    text = re.sub(r"[ \t]+", " ", text)
    return re.sub(r"\n{3,}", "\n\n", text).strip()
...
def _format_results(query: str, items: list[dict[str, Any]], n: int) -> str:
    """Отформатировать результаты поиска в текстовый вид.

    Args:
        query: Поисковый запрос.
        items: Список результатов от провайдера.
...
class WebSearchTool(Tool):
    """Инструмент веб-поиска с поддержкой нескольких провайдеров.

    Поддерживаемые провайдеры: brave, tavily, searxng, jina, duckduckgo.
    """
    ...
    async def execute(  # type: ignore[override]
        ...
        n = min(max(max_results or self.config.max_results, 1), 10)
        logger.debug("Запрос: [{}], кол-во: [{}]", query, n)
    ...
    async def _search_brave(self, query: str, n: int) -> str:
        ...
        if not api_key:
            logger.warning("BRAVE_API_KEY не установлен, использую DuckDuckGo")
            return await self._search_duckduckgo(query, n)
        try:
    ...
    async def _search_tavily(self, query: str, n: int) -> str:
        ...
        if not api_key:
            logger.warning("TAVILY_API_KEY не установлен, использую DuckDuckGo")
            return await self._search_duckduckgo(query, n)
        try:
            async with httpx.AsyncClient(proxy=self.proxy) as client:
                r = await client.post(
    ...
    async def _search_searxng(self, query: str, n: int) -> str:
        """Выполнить поиск через SearXNG API.
        ...
        if not base_url:
            logger.warning("SEARXNG_BASE_URL не установлен, использую DuckDuckGo")
            return await self._search_duckduckgo(query, n)
    ...
    async def _search_jina(self, query: str, n: int) -> str:
        """Выполнить поиск через Jina AI API.
        ...
        if not api_key:
            logger.warning("JINA_API_KEY не установлен, использую DuckDuckGo")
            return await self._search_duckduckgo(query, n)
        try:
            headers = {
                "Accept": "application/json",
    ...
    async def _search_duckduckgo(query: str, n: int) -> str:
        """Выполнить поиск через DuckDuckGo (без API ключа).
        ...
        try:
            ...
            logger.warning("Поиск DuckDuckGo завершился ошибкой: {}", e)
...
class WebFetchTool(Tool):
    ...
    Использует Jina Reader API как основной метод, с fallback на readability-lxml.
    """
    ...
    async def execute(  # type: ignore[override]
        ...
        **kwargs: Any,
        ...
        """Извлечь контент по URL.

        Args:
            url: URL для извлечения контента.
            extractMode: Режим извлечения (markdown или text).
            maxChars: Максимальное количество символов.
            **kwargs: Дополнительные аргументы.

        Returns:
            JSON с извлечённым контентом или ошибкой.
        """
    ...
    async def _fetch_jina(self, url: str, max_chars: int) -> str | None:
        """Попытаться извлечь контент через Jina Reader API.
        ...
        Returns:
            JSON-строка с результатом или None при ошибке.
        """
        try:
            ...
            async with httpx.AsyncClient(proxy=self.proxy, timeout=20.0) as client:
                ...
                if r.status_code == 429:
                    logger.debug(
                        "Jina Reader ограничен по запросам, использую readability"
                    )
                    return None
                r.raise_for_status()
            ...
            logger.debug(
                "Jina Reader не сработал для {}, использую readability: {}", url, e
            )
            return None
    ...
    async def _fetch_readability(
        ...
        """Локальный fallback для извлечения контента через readability-lxml.

        Args:
            url: URL для извлечения.
            extract_mode: Режим извлечения (markdown или text).
            max_chars: Максимальное количество символов.
